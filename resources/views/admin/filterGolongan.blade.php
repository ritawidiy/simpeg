...
@section('tittle', 'Biodata Pegawai: Filter Golongan')
@section('content')
        ...
        <div class="x_panel">
          ...
          <div class="x_content">
            <div class="row form-group">
              <div class="col-lg-12">
                <label for="filter" class="control-label">Filter Golongan</label>
                <select id="filter" data-live-search="true" name="golongan" class="form-control selectpicker" title="-- Pilih --"
                  data-max-options="1" multiple>
                  @foreach (\App\Model\Mastergolonganpangkat::all() as $item)
                  <option value="{{ $item->namagolongan }}" {{ $find != null && $find == $item->namagolongan ? 'selected' : '' }}>{{ $item->namagolongan }}</option>                      
                  @endforeach
                </select>
              </div>
            </div>
            <table id="datatable-buttons" class="table table-striped table-bordered">
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td style="vertical-align: middle"> {{ $value->skpd != ""? $value->skpd : '-' }} </td>
                  <td style="vertical-align: middle"> {{ $golongan != "" ? $golongan->namapangkat : '-' }}<br>{{ \Carbon\Carbon::parse($value->tmt_pangkat)->format('j F Y') }} </td>
...
<script>
  $(function () {
            $("#filter").on("change", function () {
                window.location.href = '{{ route('data.pegawai.Golongan', ['gol'=>'']) }}' + $(this).val()                
            });
